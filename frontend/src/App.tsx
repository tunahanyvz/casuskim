...
function App() {
  ...
  const [loadingResult, setLoadingResult] = React.useState<LoadingResult>({
    doneLoading: false,
    roomID: null,
    roomFound: undefined,
  });
  ...
  const kicked = (reason: string) => {
    switch (reason) {
      case WSC_Reason_Kicked:
        setReturnError("Oyundan Atıldınız.");
        break;
      case WSC_Reason_Room_Close:
        setReturnError("Hareketsizlik nedeniyle oda kapatıldı.");
        break;
    }
    history.push("/");
  };

  const onLoad = (roomID: string | null, roomFound: boolean | undefined) => {
    setLoadingResult({
      doneLoading: true,
      roomID: roomID,
      roomFound: roomFound,
  ...
  const onDirectRoomLoad = async (
    roomID: string | null,
    roomFound: boolean | undefined
  ) => {
    setLoadingResult({
      doneLoading: true,
      roomID: roomID,
  ...
  const onDirectLogin = (name: string) => {
    setNickname(name);
  }; // No need to set history since user is already in room
      ...
      <Switch>
        <Route exact path="/">
          {loadingResult.doneLoading ? (
            <Entry onLogin={login} error={returnError} />
          ) : (
            <Loading callbackFunc={onLoad} />
          )}
        </Route>
        <Route path="/room">
          {" "}
          {/* All properties need to be defined to send a ws request */}
          {loadingResult.doneLoading ? (
            loadingResult.roomFound == true && roomID ? (
              ...
              <NotFoundPage message="Oda bulunamadı." />
            )
          ) : (
