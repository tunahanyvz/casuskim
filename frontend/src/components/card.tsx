...
export function Card({ spy, location, role, onOpen }: CardProps) {
  ...
  const handleResize = () => {
    if (parentRef.current && childRef.current) {
      if (parentRef.current.offsetHeight < 420) {
        childRef.current.style.transform = `scale(${
          parentRef.current.offsetHeight / childRef.current.offsetHeight
        })`;
      }
    }
  };
  ...
  return (
    ...
    >
      <div
        ref={childRef}
        className={`card-container ${spy ? "spy" : ""} ${
          flipped ? " flipped" : ""
        }`}
        onClick={flip}
      >
    ...
    <div className="card-back">
      {spy ? (
        <div>
          ...
          >
            Sen casussun. Herkesle uyum sağlamaya çalışın.
          </p>
        </div>
      ) : (
        <div
        ...
        >
          <img
            ...
            src={topSecret}
            width="50%"
            style={{ position: "absolute", top: "2.66rem" }}
          />
          <p style={{ color: "#dddddd", marginTop: "6.66rem" }}>Konum:</p>
          <p style={{ color: "white", fontSize: "2rem" }}>
            {location}
            <a
              target="_blank"
              href={`https://www.google.com/search?q=${location}+nedir`}
              style={{
                marginLeft: "0.33rem",
                color: "#aaaaaa",
                fontSize: "1.33rem",
          ...
          {role ? (
            <p style={{ color: "#dddddd", marginTop: "1.33rem" }}>Rol:</p>
          ) : null}
