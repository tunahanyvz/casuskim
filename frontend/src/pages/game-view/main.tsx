...
export const PrimaryActionButton = withStyles({
  root: {
    position: "absolute",
    left: "50%",
          ...
          {!state.endGame && state.started ? (
            <div className="topSector">
              <div className="gameTimer">
                {state.timerLength && state.started && !state.endGame ? (
                  <Timer duration={state.timerLength} />
                ) : null}
              </div>
            </div>
          ) : null}
          ...
          {sortedPlayers.map((player, _) => {
            return (
              <PlayerElement
                ...
                onKick={(discriminator: number) =>
                  send.kickPlayer(discriminator)
                }
              />
            );
          })}
