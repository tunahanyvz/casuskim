  ...
  if (data.create) {
    ...
    if (roomSearch == undefined || data.roomPass !== roomSearch.GetPassword()) {
      res
        .status(404)
        .send(
          `{"error":"Oda bulunamadı veya şifre yanlış.","mistake":3}`
        );
      return;
    }
    if (roomSearch.HasStarted()) {
      res
        .status(403)
        .send(`{"error":"Oda çoktan başladı. Daha sonra katılmayı deneyin."}`);
      return;
    }
    if (roomSearch.GetClientCount() >= MAX_ROOM_MEMBER_COUNT) {
      res.status(403).send(`{"error":"Oda dolu."}`);
      return;
    }
...
router.post("/api/join", async function (req, res) {
  ...
  if (!room) {
    res.status(400).send(`{"error":"Oda bulunamadı."}`);
    return;
