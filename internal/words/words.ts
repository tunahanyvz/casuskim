    ...
    try {
      if (line.startsWith("#")) {
        ...
        if (colonIndex === -1)
          return CreateError(
            i,
            "N/A",
            "Tanım ayrıştırılırken hata oluştu: iki nokta üst üste belirtilmedi"
          );
      ...
      if (colonIndex === -1)
        return CreateError(
          i,
          "N/A",
          "Konum ayrıştırılırken hata oluştu: iki nokta üst üste belirtilmedi"
        );
      ...
      if (location.length === 0)
        return CreateError(
          i,
          "N/A",
          "Konum ayrıştırılırken hata oluştu: konum adı boş olamaz"
        );
      ...
      for (const roleString of roleArray) {
        ...
        if (final.length > MAX_ROLE_LENGTH)
          return CreateError(
            i,
            location,
            `Roller ayrıştırılırken hata oluştu: rol '${final.substr(
              0,
              MAX_ROLE_LENGTH
            )}...' too long`
          );
        ...
        if (roleString.length !== final.length && final.length === 0)
          return CreateError(
            i,
            location,
            `Roller ayrıştırılırken hata oluştu: rol boş olamaz`
          );
        roles.push(final);
      }
      ...
      return CreateError(i, "N/A", "Bilinmeyen bir ayrıştırma hatası oluştu");
    }
  }
...
export const CreatePack = (list: ListResult): Pack => {
  let sumLocations = 0,
    sumRoles = 0;
  if (!list.data) throw "Herhangi bir veri olmadan bir paket oluşturulmaya çalışılıyor!";
